#!/usr/bin/env bash
#SBATCH --job-name=rl-langgraph-8B-opt
...
# ================= Work directories on /data1 (more space) =================
...
# ================= cluster topology =================
export GPUS_PER_NODE=${SLURM_GPUS_ON_NODE:-${GPUS_PER_NODE:-8}}  # Use 8 GPUs for 8B model
...
# Use local Qwen2.5-3B model for testing
...
experiment_name=qwen2.5-3b-test
...
ppo_mini_batch_size=1  # Reduced from 2 for lower memory
n_resp_per_prompt=2
...
# as it's incompatible with vLLM's memory pool

infer_tp=8  # vLLM tensor parallel size - use all 8 GPUs to reduce per-GPU memory
train_sp=8  # Ulysses sequence parallel size for actor (must match GPU count)

# ================= FSDP Optimization =================
# Key change: Use FSDP2 with offload_policy for better memory management
fsdp_strategy=fsdp2
offload_policy=true  # FSDP2-specific: offload params/grads/optimizer during training
param_offload=false  # Disabled because offload_policy handles this
optimizer_offload=false  # Disabled because offload_policy handles this

# ================= vLLM Memory Optimization =================
# In async mode, actor model is initialized first (~19.5 GiB), leaving only ~4 GiB free.
# vLLM must fit in this remaining space. Reduce utilization significantly.
gpu_memory_utilization=0.3  # Increased for 3B model to support larger context
max_model_len=2048  # Increased to handle multi-turn dialogue accumulation
