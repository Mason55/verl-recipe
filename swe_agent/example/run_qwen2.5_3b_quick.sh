...
echo "=========================================="
echo "SWE Agent Quick Test - Using run_qwen2.5_3b.sh Config"
echo "=========================================="
echo "==========================================="
...
# ========== 训练参数 ==========
max_turns=15              # 给 agent 足够轮数完成任务+submit（含 ~5 轮格式重试）
max_prompt_length=4096    # prompt 实际 ~300 token，但 padding 上限需留余量
max_response_length=16384 # 多轮对话累积 response 较长（含重试），16k 减少截断
actor_lr=5e-6             # GRPO 通常需要比 PPO 更高的 LR 才能学到信号

train_batch_size=8        # 需要匹配 agent_loop_workers 数量
ppo_mini_batch_size=4     # mini_batch < batch: 每 step 多次梯度更新，更充分利用数据
n_resp_per_prompt=1
...
gpu_memory_utilization=0.4   # 每卡 24GB，训练峰值 ~13.5GB，vLLM 可用 ~40%
# max_model_len: vLLM 最大序列长度，需 >= prompt + response 生成长度
max_model_len=16384          # 16k 支持较长的多轮交互

# prompt_length 用于 rollout padding，须 <= max_model_len
# actor_max_token_len_per_gpu 须 >= (prompt_length + response_length)
