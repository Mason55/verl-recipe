...
class AgentConfigValidator:
    ...
    def validate(self) -> None:
        ...
        if self.config is None:
            raise ConfigValidationError(
                "SWE Agent Loop 需要 agent 配置。"
                "请在 actor_rollout_ref.rollout.agent 中提供配置。"
            )
    ...
    def _validate_sandbox_config(self) -> None:
        ...
        if not isinstance(sandbox_config, dict):
            raise ConfigValidationError("sandbox_config 必须是字典类型")
        ...
        if swe_agent_timeout is not None:
            if not isinstance(swe_agent_timeout, (int, float)) or swe_agent_timeout <= 0:
                raise ConfigValidationError(
                    f"sandbox_config.swe_agent_timeout 必须是正数，当前值: {swe_agent_timeout}"
                )
        ...
        if max_steps is not None:
            if not isinstance(max_steps, int) or max_steps <= 0:
                raise ConfigValidationError(
                    f"sandbox_config.max_steps 必须是正整数，当前值: {max_steps}"
                )
